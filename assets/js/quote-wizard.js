(function () {
  ...
  var QUESTIONS = [
    ...
    },
    {
      ...
      options: [
        { id: '201', label: 'In house — we clean our own facility, but looking to hire a cleaner' },
        { id: '202', label: 'Outsourced — we pay a cleaning company' },
        { id: '203', label: 'We are a new congregation — we are looking for a cleaner' },
        { id: '204', label: 'We are relocating our current facility' },
  ...
  var QUOTES_QUESTION = {
    ...
    hint: 'Most facilities pick 3 — enough to compare without being overwhelming.',
    options: [
  ...
  function prefillFromQuery() {
    ...
    var facility = (params.get('facility') || '').trim();
    if (facility) state.customer.company_name = facility;
  ...
  function renderDetails(host) {
    ...
    var fields = [
      { key: 'company_name', label: 'Church or ministry name', type: 'text', autocomplete: 'organization' },
      ...
      { key: 'position', label: 'Your role', type: 'text', autocomplete: 'organization-title' },
  ...
  function validateDetails() {
    ...
    if (!c.company_name.trim()) return 'Enter your church or ministry name.';
    if (!c.first_name.trim()) return 'Enter your first name.';
    if (!c.last_name.trim()) return 'Enter your last name.';
    if (!c.position.trim()) return 'Enter your role.';
  ...
  function renderReview(host) {
    ...
    company.appendChild(el('h4', null, 'Church details'));
    var companyRows = [
      ['Church / ministry', state.customer.company_name],
      ['Contact', state.customer.first_name + ' ' + state.customer.last_name],
      ['Role', state.customer.position],
